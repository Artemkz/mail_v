...
- **Сбор в один ящик** — копирование писем со всех подключённых ящиков в один целевой IMAP-ящик (ящик-сборник)
- **Расширенная работа с письмами** — непрочитанные, избранное, архив, массовые действия, ответ/пересылка, перемещение в папку
...
### Собрать все ящики в один (IMAP APPEND)

1. Отметьте целевой ящик: `PATCH /api/mailboxes/{id}/consolidation-target`
2. Запустите консолидацию:

```bash
curl -X PATCH "http://localhost:8000/api/mailboxes/1/consolidation-target"
